<div class="w-full max-w-6xl mx-auto md:p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        @if (!$showAddForm)
            <button wire:click="showAddVariantForm"
    ...
    @if ($showAddForm)
        <div class="bg-white rounded-xl border-gray-100 p-4 mb-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @foreach ($availableVariants as $variant)
                        <label
                            ...
                            <input type="checkbox" value="{{ $variant->id }}" wire:model.live="selectedVariantTypes"
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        @foreach ($existingImages as $index => $image)
                            <div class="relative group">
                                <img src="{{ $image['image_path'] }}" alt="Current Image"
                                    class="w-full h-24 object-cover rounded-lg border border-gray-200">
                                @if ($image['is_primary'])
                                    <span
                                        class="absolute top-2 left-2 bg-[#8f4da7] text-white text-xs px-2 py-1 rounded">Featured</span>
                                @endif
            ...
            <div class="mb-6">
                ...
                <div
                    class="border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-[#8f4da7] transition-colors">
                    @if ($editingCombination !== null ? $editing_featured_image_preview : $new_featured_image_preview)
                        <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                            <img src="{{ $editingCombination !== null ? $editing_featured_image_preview : $new_featured_image_preview }}"
                                alt="Preview" class="w-24 h-24 object-cover rounded-lg border border-gray-200">
                            <button type="button"
                    ...
                    @elseif ($editingCombination !== null && $existingImages)
                        ...
                        @if ($featured)
                            <div class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                                <img src="{{ $featured['image_path'] }}" alt="Current Featured Image"
            ...
            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button wire:click="addCombination"
                    ...
                    <span wire:loading.remove
                        wire:target="addCombination">{{ $editingCombination !== null ? 'Update' : 'Add' }}
                        Variant</span>
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-shadow">
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        <div class="flex items-start sm:items-center gap-4 flex-1 min-w-0">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap gap-2 mb-2">
                                    @php
                                        $variantValues =
                                            $combination['variant_values_data'] ??
                                    ...
                                    @foreach ($variantValues as $name => $value)
                                        <span
                                            class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#f3e8f7] text-[#8f4da7]">
                                            {{ $name }}: {{ $value }}
                                        </span>
                                    @endforeach
                                </div>
                                ...
                                <div class="text-sm text-gray-500 space-y-1 sm:space-y-0 sm:space-x-4">
                                    @if ($combination['price'])
                                        <span
                                            class="inline-block">₹{{ number_format($combination['price'], 2) }}</span>
                                    @endif
                        ...
                        <div class="flex gap-2 self-end sm:self-auto">
                            <button wire:click="editCombination({{ $index }})"
                            ...
                            <button wire:click="deleteCombination({{ $index }})"
                    ...
                    @if (isset($combination['images']) && ($gallery = collect($combination['images'])->where('is_primary', false)->take(3)))
                        <div class="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                            ...
                            @if (count($combination['images']) > 4)
                                <div
                                    class="w-10 h-10 bg-gray-100 rounded border border-gray-200 flex items-center justify-center text-xs text-gray-500">
