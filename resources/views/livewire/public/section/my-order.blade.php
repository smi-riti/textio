<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    @foreach (['error' => 'red', 'success' => 'green', 'message' => 'green'] as $type => $color)
        @if (session($type))
            <div class="bg-{{ $color }}-50 border-l-4 border-{{ $color }}-400 text-{{ $color }}-600 p-4 mb-6 rounded" role="alert">
                <p>{{ session($type) }}</p>
            </div>
        @endif
    @endforeach
                    ...
                    @foreach ($addresses->take(3) as $address)
                        <div class="flex flex-col sm:flex-row justify-between gap-2 p-4 mb-3 border rounded-lg">
                            <label class="flex items-start cursor-pointer flex-1">
                                <input type="radio" wire:model="addressId" value="{{ $address->id }}" class="h-5 w-5 text-primary mt-1" required>
        ...
        <div class="ml-11 mt-4 space-y-3">
            @foreach (['Cash on Delivery', 'UPI'] as $method)
                <label class="flex items-center cursor-pointer">
                    <input type="radio" wire:model="paymentMethod" value="{{ $method }}" class="h-5 w-5 text-primary" required>
    ...
    <div class="sticky top-6 bg-white rounded-lg p-6 border border-gray-200">
        ...
        <div class="space-y-3 text-sm sm:text-base">
            <div class="flex justify-between">
                <span class="text-gray-700">Price ({{ $cartItems->count() }} item{{ $cartItems->count() > 1 ? 's' : '' }})</span>
                <span class="text-gray-900">
                    ₹{{ number_format($cartItems->sum(fn($i) => $i['product']['discount_price'] * $i['quantity']), 2) }}
                </span>
            </div>
            <div class="flex justify-between text-green-600">
                <span>Product Discount</span>
                <span>₹{{ number_format($cartItems->sum(fn($i) => ($i['product']['price'] - $i['product']['discount_price']) * $i['quantity']), 2) }}</span>
            </div>
            @if ($discountAmount > 0)
                <div class="flex justify-between text-green-600">
        ...
        <p class="text-green-600 mt-4 text-sm sm:text-base">
            You will save ₹{{ number_format($cartItems->sum(fn($i) => ($i['product']['price'] - $i['product']['discount_price']) * $i['quantity']) + $discountAmount, 2) }} on this order
        </p>

        <livewire:public.section.coupon.apply-coupon :cart-total="$cartItems->sum(fn($i) => $i['product']['discount_price'] * $i['quantity'])" />
    </div>
</div>
</div>
</div>
